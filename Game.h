...
public:
	...
	//If the game isn't over (i.e., more moves are possible), set over to false and 
	//do not change anything else. 
	//Otherwise, set over to true and hasWinner to true if the game has a winner,
	//or false if it resulted in a tie. 
	...
	//scrolling through the whole game, it would be reasonable periodically
	//to prompt the viewer to press ENTER to continue and not proceed until ENTER 
	//is pressed. (The ignore function for input streams is useful here.) 
	//Announce the winner at the end of the game. You can apportion to your liking the responsibility for displaying the board between this function and the move function. 
...
private:
	...
	Player *m_north;
	bool over = false;
	bool hasWinner = false; //no winner initially
	Side winner;
	Side m_turn = SOUTH;
	Side endSide;
	int endHole;
